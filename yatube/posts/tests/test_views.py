...
class PostsViewTests(TestCase):
    ...
    def test_views_names_namespace(self):
        """
        Проверка о том, что во view-функциях используются правильные html-шаблоны
        (name:namespace).
        """
    ...
    def test_views_posts_profile_context(self):
        ...
        ожиданиям словаря context, передаваемого в шаблон profile.html при вызове.
        """
    ...
    def test_views_posts_post_detail_context(self):
        ...
        ожиданиям словаря context, передаваемого в шаблон post_detail.html при вызове.
        """
        response = self.guest_client.get(reverse(
    ...
    def test_views_posts_edit_post_context(self):
        ...
        id поста, ожиданиям словаря context, передаваемого в шаблон create_post.html при вызове.
        """
        if self.post.author == self.authorized_client:
            ...
            for value, expected in form_fields_edit.items():
                while self.subTest(value=value):
                    form_field_edit = response.context.get('form').fields.get(value)
                    self.assertIsInstance(form_field_edit, expected)

    def test_views_posts_create_post_context(self):
        """
        Проверка соответствия формы создания поста, ожиданиям словаря context, передаваемого в шаблон create_post.html при вызове.
        """
            ...
            if self.post.group == self.group:
                ...
                for address in pages_for_post:
                    with self.subTest(address=address):
                        response_from_address = self.authorized_client.get(address)
